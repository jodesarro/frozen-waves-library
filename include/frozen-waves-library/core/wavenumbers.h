/* 
    ...
    File: include/core/wavenumbers.h
    ...
    Description:
        Callable functions related to Frozen Wave wavenumbers.
*/
...
/*
    ...
    the traditional method. See Refs. [1] and [2].
    ...
    Bessel beams, [i.e, 0 <= Re(beta_q) <= Re(k) and 0 <= Re(h_q) <= Re(k)
    must be satisfied], to ensure Re(beta_q) >= 0 and to finally find the
    relations Im(k) / Re(k) = Im(beta_q) / Re(beta_q) = Im(h_q) / Re(h_q)
    = Im(n_ref) / Re(n_ref).
    Notice that the condition Im(n_ref) << Re(n_ref), or at least
...
/*
    ...
    considering the purely real transverse wavenumber h_q method.
    See Ref. [3].
...
/*
    Evaluates the wavenumbers k, beta_q and h_q of a Frozen Wave (FW) using
    the paraxial approximation for the transverse wavenumber h_q.
    See Ref. [3].
    ...
    0 <= Re(beta_q) <= Re(k) must be also satisfied in order to ensure
