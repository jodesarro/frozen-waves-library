/* 
    ...
    Description:
        Callable functions related to Frozen Wave coefficients.
*/
...
/*
    Evaluates the A_q coefficients of a Frozen Wave (FW), resistant or not to
    a possible absorption due to a medium with complex refractive index.
    ...
    Implementation: The
    A_q = int_0^L F(z) exp(-betabar_0) exp(j*2*pi*q*z/L) dz,
    where betabar_0 = imag(beta_0) if absorption_resistant = true or
    betabar_0 = 0 if absorption_resistant = false,
...
/*
    ...
    restricted to the case where all FWs share the same N, Q and P
    (i.e., for 1 <= p <= P, N_p = N, Q_p = Q, and L_p = L), resistant or not
    to a possible absorption due to a medium with complex refractive index.
    See Refs. [5], [6] and [7].
    ...
    y -> iy, z -> iz * L / (izmax - 1), 0<=iy<iymax, and 0<=iz<izmax.
    ...
    the A_q,p coefficients, where
    A_q,p -> A[iq + iqmax*ip], p -> ip + 1, q -> iq - N, 0<=ip<ipmax,
    and 0<=iq<iqmax.
    - absorption_resistant, if false, the FW is not absorption-resistant.

    Implementation: The
    A_q = int_0^L F(y=y_0p, z) exp(-betabar_0) exp(j*2*pi*q*z/L) dz,
    where y_0p -> iy * (ipmax - 1) / (iymax - 1),
    betabar_0 = imag(beta_0) if absorption_resistant = true or
    betabar_0 = 0 if absorption_resistant = false,
    is computed by means of an approximation of the
    integral by the trapezoidal method for equally spaced z for a given
...
/*
    ...
    FWs, restricted to the case where all FWs share the same N, Q and P
    (i.e., for 1 <= p <= P and 1 <= s <= S, N_sp = N, Q_sp = Q, and L_sp = L),
    ...
    F(x,y,z), where F(x,y,z) -> F[iz + izmax*iy + izmax*iymax*ix],
    x -> ix, y -> iy, z -> iz * L / (izmax - 1), 0<=ix<ixmax, 0<=iy<iymax,
    and 0<=iz<izmax.
    ...
    to output the A_q,sp coefficients,
    where A_q,sp -> A[iq + iqmax*ip + iqmax*ipmax*is], s -> is + 1,
    p -> ip + 1, q -> iq - N, 0<=is<ismax, 0<=ip<ipmax, and 0<=iq<iqmax.
    - absorption_resistant, if false, the FW is not absorption-resistant.

    Implementation: The
    A_q = int_0^L F(x=x_0s, y=y_0p, z) exp(-betabar_0) exp(j*2*pi*q*z/L) dz,
    where x_0s -> ix * (ismax - 1) / (ixmax - 1),
    y_0p -> iy * (ipmax - 1) / (iymax - 1),
    betabar_0 = imag(beta_0) if absorption_resistant = true or
    betabar_0 = 0 if absorption_resistant = false,
    is computed by means of an approximation of the
    integral by the trapezoidal method for equally spaced z for a given
